...
const RenderTable = () => {
  ...
  const update = (id: number) =>{
    const updatedData = data.map(
      (item) => item.id == id ?
      {...item, label: itemName, price: itemPrice}
      : item
    );
    setData(updatedData);
    setItemID(null);
  ...
  const removeItem = (id: number) => {
    ...
    const reorder = updatedData.map((item, index) => ({
      ...item, id:index+1, //should reassign all ids starting from 1
    }));
  ...
  const addItem = () => {
    const newID = data.length > 0 ? Math.max(...data.map(item => item.id)) : 0;
  ...
  const renderVewOnlyItem = ({item}: {item: ListItemData}) => (
    <ListItem bottomDivider>
      ...
      <ListItem.Content>
        {item.label == "Total" ? (
            <ListItem.Title style={{ fontWeight: "bold" }}>{item.label}</ListItem.Title>
          ) : (
      ...
      {item.label !== "Total" ? (
        <ListItem.Swipeable
          ...
          {itemID != item.id ? 
            (
              <Button
                icon = {{name: 'edit', type: 'material', size: 20, color: 'white',}}
                buttonStyle = {{backgroundColor: "#32425f", borderRadius: 20,}}
                onPress={()=>editValues(item)}
              />
            ):(
              <Button
              icon = {{name: 'done-outline', type: 'material', size: 20, color: 'white',}}
